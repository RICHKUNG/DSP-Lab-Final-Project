...
from typing import Optional, Tuple, Callable, Dict, Iterable
...
TemplateAdder = Callable[[str, np.ndarray, str], None]
...
def load_templates_from_dir(
    ...
    Args:
        template_dir: 模板目錄路徑
        add_template: 新增模板的回調函數 (command, audio, filename)
        add_noise: 新增噪音的回調函數 (audio)
        noise_decider: 判斷是否為噪音檔的函數
        command_mapping: 中文→英文指令對應

    Returns:
        (num_templates, num_noise) 載入的數量
    """
    ...
    # 4) 說話者子目錄
    skip_dirs = {"features", "raw", "cmd_templates", "noise", "record",
                 "src", "temp", "__pycache__", ".git"}
